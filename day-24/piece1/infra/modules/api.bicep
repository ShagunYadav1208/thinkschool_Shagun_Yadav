...
//
// Carries an `azd-service-name` tag (new for this piece, day-23's version
// didn't need one) - that's how `azd deploy`/`azd up` find this exact site
// to publish QuotesApi's build output to, without azure.yaml having to name
// a resource explicitly. See azure.yaml's `services.api`.
...
@description('azd service name this site answers to - must match a key under `services` in azure.yaml.')
param azdServiceName string = 'api'
