 ...
 * unreliable in day-25: after a real, completed sign-in, clicking into a
 * guarded route sometimes bounced straight back instead of showing the
 * detail - no error, just silently treated as "not authenticated" even
 * though `AuthService.isAuthenticated()` (the same signal every other part
 * of this app trusts) was true the whole time. This guard sidesteps that
 * state machine entirely and just reads the one signal already trusted
 * elsewhere.
 *
