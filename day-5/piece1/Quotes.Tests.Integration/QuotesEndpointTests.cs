...
public sealed class QuotesEndpointTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task GetAuthorStats_WithMultipleQuotesPerAuthor_ReturnsCorrectCountPerRow()
    {
        await CreateQuoteAsync("Ada Lovelace", "First Ada quote.");
        await CreateQuoteAsync("Ada Lovelace", "Second Ada quote.");
        await CreateQuoteAsync("Grace Hopper", "Only Grace quote.");

        var response = await _client.GetAsync("/api/quotes/author-stats");

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);

        var stats = await response.Content.ReadFromJsonAsync<List<AuthorStatDto>>();
        Assert.NotNull(stats);
        Assert.Equal(3, stats!.Count);
        Assert.All(stats.Where(s => s.Author == "Ada Lovelace"), s => Assert.Equal(2, s.AuthorQuoteCount));
        Assert.All(stats.Where(s => s.Author == "Grace Hopper"), s => Assert.Equal(1, s.AuthorQuoteCount));
    }
    ...
    private sealed record AuthorStatDto(int Id, string Author, string Text, int AuthorQuoteCount);
}
