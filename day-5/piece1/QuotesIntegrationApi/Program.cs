...
{
    message = "Day 5 Piece 1: diagnosing a slow endpoint with OpenTelemetry traces.",
    endpoints = new[]
    {
        "POST /auth/token",
        "GET /api/quotes",
...
// Fixed: was an N+1 query (one COUNT per row, 301 round trips for 300 quotes — see
// evidence/trace-before-summary.json). Every author's count now comes back in the same
// GROUP BY round trip that produced the list, so this is 2 queries total regardless of how
// many quotes exist, not 1 + N. See evidence/trace-after-summary.json for the fixed trace.
quotes.MapGet("/author-stats", async (
    QuotesDbContext db,
    CancellationToken cancellationToken) =>
{
    var allQuotes = await db.Quotes
        .AsNoTracking()
        .OrderBy(q => q.Id)
        .ToListAsync(cancellationToken);
    ...
    var results = allQuotes.Select(quote => new
    {
        ...
        quote.Author,
        quote.Text,
        AuthorQuoteCount = authorCounts.GetValueOrDefault(quote.Author, 0)
    });
