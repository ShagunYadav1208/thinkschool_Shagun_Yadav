// Infrastructure carried over unchanged from Day 4 Piece 5: an Application Insights resource
// (workspace-based, the only kind Azure creates today), an email action group, and a log alert
// on POST /api/quotes latency. Day 5 Piece 1's own deliverable is the KQL in queries.kql for
// catching the N+1 endpoint diagnosed in README.md — this file just keeps the app's existing
// App Insights wiring (Program.cs) deployable.
//
