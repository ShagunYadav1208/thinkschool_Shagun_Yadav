...
// GET /api/quotes/ with no page/size silently defaults to page=1, size=10,
// oldest-first - with more than 10 quotes, a newly-created one (highest id)
// falls past that window and the next poll below would overwrite it right
// back out of `quotes`. 100 is the API's own max `size`; requesting it here
// keeps the "everything, live" contract this store's callers (Explore/All
// Quotes) actually expect.
const LIST_POLL_SIZE = 100;
  ...
  start(): void {
    ...
    interval(HEALTH_CHECK_INTERVAL_MS)
      .pipe(
        startWith(0),
        switchMap(() =>
          this.quotesService.getQuotesPage(1, LIST_POLL_SIZE).pipe(
