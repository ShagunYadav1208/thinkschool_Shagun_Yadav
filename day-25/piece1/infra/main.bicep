// Day 25 / Piece 1 infrastructure: day-24's API + Azure SQL + Service Bus
// stack (still provisioned via `azd provision` / Deployment Stacks), plus a
// Key Vault for the one config value in this app that's actually
// secret-shaped (Redis:ConnectionString) - see README "MI wiring + a Key
// Vault reference" for the full story and "Current status" for what was
// actually run.
//
  ...
  params: {
    ...
    extraAppSettings: [
      ...
      }
      {
        ...
        // from a different day's exercise. No password: "Authentication=
        // Active Directory Managed Identity" is the entire auth story.
